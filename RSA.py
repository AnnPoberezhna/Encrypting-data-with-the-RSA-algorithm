...
def encrypt_png_file(input_path, output_path, public_key, mode='ecb'):
    """
    Encrypts a PNG file at the pixel level using raw RSA in the selected mode.

    Supported modes:
      'ecb' — Electronic CodeBook: each block encrypted independently (default).
      'cbc' — Cipher Block Chaining: each block XORed with previous ciphertext.
      'ctr' — Counter mode: keystream generated by encrypting a counter with RSA.

    Steps:
      1. Decompress IDAT → raw scanlines (filter_byte + pixel_bytes per row).
      2. Extract pixel bytes; keep filter bytes for recovery.
      3. Encrypt pixel bytes with the chosen mode.
      4. Pack ciphertext back as filter-0 scanlines and recompress → new IDAT.
      5. Store mode, public exponent e, original height, pixel length and filter
         bytes in a private 'rsFl' chunk so decryption can restore the file
         byte-for-byte.
    ...
    print(f"[*] Encrypting PNG (pixel-level, raw RSA, {mode.upper()} mode): {input_path}")
    ...
    if mode == 'cbc':
        encrypted = encrypt_data_cbc(payload, e, n)
    elif mode == 'ctr':
        encrypted = encrypt_data_ctr(payload, e, n)
    else:
        encrypted = encrypt_data(payload, e, n)  # ECB
    ...
    # rsFl chunk layout (new format, magic b'rsFL'):
    #   4 bytes  magic b'rsFL'
    #   3 bytes  mode ASCII (e.g. b'ecb'), padded with b'\x00' to 4 bytes total
    #   1 byte   padding (b'\x00')
    #   4 bytes  length of e in bytes (big-endian)
    #   N bytes  public exponent e (big-endian); needed by CTR decryption
    #   4 bytes  orig_h
    #   4 bytes  orig_pixel_len
    #   rest     filter_bytes
    mode_tag = mode.encode('ascii').ljust(3, b'\x00')[:3] + b'\x00'
    e_bytes  = e.to_bytes((e.bit_length() + 7) // 8, 'big')
    recovery = (b'rsFL' + mode_tag
                + struct.pack('>I', len(e_bytes)) + e_bytes
                + struct.pack('>II', orig_h, len(pixel_bytes))
                + filter_bytes)
...
def decrypt_png_file(input_path, output_path, private_key):
    ...
    Supports ECB, CBC and CTR modes — the mode is read from the 'rsFl' chunk.
    ...
    # Detect chunk format version by magic bytes.
    # New format starts with b'rsFL'; legacy (ECB-only) format starts with orig_h.
    if recovery[:4] == b'rsFL':
        mode       = recovery[4:7].rstrip(b'\x00').decode('ascii')
        e_len      = struct.unpack('>I', recovery[8:12])[0]
        e          = int.from_bytes(recovery[12:12 + e_len], 'big')
        off        = 12 + e_len
        orig_h         = struct.unpack('>I', recovery[off:off + 4])[0]
        orig_pixel_len = struct.unpack('>I', recovery[off + 4:off + 8])[0]
        filter_bytes   = recovery[off + 8:]
    else:
        # Legacy ECB format: [orig_h 4B][orig_pixel_len 4B][filter_bytes]
        mode           = 'ecb'
        e              = None
    ...
    if mode == 'cbc':
        # CBC ciphertext = IV (cipher_block_size) + ceil(payload/block_size) encrypted blocks
        num_pt_blocks = (payload_len + block_size - 1) // block_size
        cbc_len       = (1 + num_pt_blocks) * cipher_block_size
        full_payload  = decrypt_data_cbc(enc_bytes[:cbc_len], d, n)
    elif mode == 'ctr':
        # CTR ciphertext = nonce (block_size//2) + plaintext-length bytes
        nonce_len    = block_size // 2
        ctr_len      = nonce_len + payload_len
        full_payload = decrypt_data_ctr(enc_bytes[:ctr_len], e, n)
    else:
        # ECB — manual block decryption to preserve any trailing null bytes
        num_blocks = (payload_len + block_size - 1) // block_size
        trimmed    = enc_bytes[:num_blocks * cipher_block_size]
        plaintext_blocks = []
        for i in range(0, len(trimmed), cipher_block_size):
            block = trimmed[i:i + cipher_block_size]
